...
class BillSell(QtWidgets.QDialog):
    ...
    def c_name_changed(self):
        ...
        self.c_phone.setText(
            database.db.get_phone_by_name("customer", self.c_name.currentText())
        )
    ...
    def fill_bill(self, id):
        ...
        for row_idx, row in enumerate(orders):
            ...
            self.bs_table.setItem(
                row_idx, 1, QtWidgets.QTableWidgetItem(str(row["name"]))
            )
            self.bs_table.setItem(
                row_idx, 2, QtWidgets.QTableWidgetItem(str(row["quantity"]))
            )
            if self.bill_type.currentIndex() == 0:
                self.bs_table.setItem(
                    row_idx, 3, QtWidgets.QTableWidgetItem(str(row["sell_price"]))
                )
            else:
                self.bs_table.setItem(
                    row_idx, 3, QtWidgets.QTableWidgetItem(str(row["sell_price_wh"]))
                )
            format_float = round(
                float(self.bs_table.item(row_idx, 3).text()) * float(bill["dollar_tr"]),
                2,
            )
            self.bs_table.setItem(
                row_idx, 4, QtWidgets.QTableWidgetItem(str(format_float))
            )
            self.bs_table.setItem(
                row_idx, 5, QtWidgets.QTableWidgetItem(str(row["discount"]))
            )
            self.bs_table.setItem(
        ...
        if int(product["quantity"]) >= 1:
            for idx in range(self.bs_table.rowCount() - 1):
                if (
                    ...
                    self.bs_table.setItem(
                        idx, 6, QtWidgets.QTableWidgetItem(str(total_d))
                    )
                    total_t = new * float(self.bs_table.item(idx, 4).text())
                    total_t = round(total_t, 2)
                    self.bs_table.setItem(
                        idx, 7, QtWidgets.QTableWidgetItem(str(total_t))
                    )
                    self.bs_table.setItem(
                        current_row, 0, QtWidgets.QTableWidgetItem("")
                    )
                    self.delete_order(current_row)
                    return
            ...
            self.bs_table.setItem(
                current_row, 0, QtWidgets.QTableWidgetItem(product["code"])
            )
            self.bs_table.setItem(
                current_row, 1, QtWidgets.QTableWidgetItem(product["name"])
            )
            ...
            if self.bill_type.currentIndex() == 0:
                self.bs_table.setItem(
                    current_row,
                    3,
                    QtWidgets.QTableWidgetItem(str(product["sell_price"])),
                )
            else:
                self.bs_table.setItem(
                    current_row,
                    3,
                    QtWidgets.QTableWidgetItem(str(product["sell_price_wh"])),
                )
            format_float = round(
                float(self.bs_table.item(current_row, 3).text())
                * float(self.d_tr.text()),
                2,
            )
            self.bs_table.setItem(
                current_row, 4, QtWidgets.QTableWidgetItem(str(format_float))
            )
            self.bs_table.setItem(current_row, 5, QtWidgets.QTableWidgetItem("0"))
            self.bs_table.setItem(
                current_row,
                6,
                QtWidgets.QTableWidgetItem(self.bs_table.item(current_row, 3).text()),
            )
            self.bs_table.setItem(
                current_row, 7, QtWidgets.QTableWidgetItem(str(format_float))
            )
    ...
    def enter_event(self, current_row):
        ...
        if quantity > int(product["quantity"]):
            ...
            self.bs_table.setItem(
                current_row, 2, QtWidgets.QTableWidgetItem(product["quantity"])
            )
    ...
    def discount_on_press(self, x):
        ...
        if x == "d":
            ...
            self.last_total_t.setText(str(t_float))
            self.discount_d.setText(
                str(round(float(self.discount_t.text()) / DOLLAR, 2))
            )
            d_float = round(
                float(self.total_d.text()) - float(self.discount_d.text()), 2
            )
        ...
        for idx in range(self.bs_table.rowCount()):
            ...
            if self.bs_table.item(idx, 0) and self.bs_table.item(idx, 0).text():
                if hasattr(self.bs_table.item(idx, 0), "id"):
                    ...
                    order["p_id"] = database.db.get_id_by_code(
                        "product", self.bs_table.item(idx, 0).text()
                    )
...
class BillBuy(QtWidgets.QDialog):
    ...
    def s_name_changed(self):
        if self.s_name.currentText() == "":
            ...
            self.s_phone.setText(
                database.db.get_phone_by_name("supplier", self.s_name.currentText())
            )
    ...
    def fill_bill(self, id):
        if id == 0:
            ...
            self.b_date.setDate(QDate(bill["date"]))
            self.s_name.setCurrentText(
                database.db.get_name_by_id("supplier", bill["s_id"])
            )
        ...
        for row_idx, row in enumerate(orders):
            ...
            self.bb_table.setItem(
                row_idx, 3, QtWidgets.QTableWidgetItem(str(row["buy_price"]))
            )
            self.bb_table.setItem(
                row_idx, 4, QtWidgets.QTableWidgetItem(str(row["total"]))
            )
            btn_delete = QtWidgets.QPushButton(QtGui.QIcon.fromTheme("delete"), "")
            btn_delete.clicked.connect(
    ...
    def update_table(self, current_row):
        ...
        self.bb_table.setItem(
            current_row, 3, QtWidgets.QTableWidgetItem(str(product["buy_price"]))
        )
        self.bb_table.setItem(
            current_row, 4, QtWidgets.QTableWidgetItem(str(product["buy_price"]))
        )
    ...
    def discount_on_press(self):
        self.last_total.setText(
            str(float(self.total.text()) - float(self.discount.text()))
        )
        ...
        for idx in range(self.bb_table.rowCount()):
            ...
            if self.bb_table.item(idx, 0) and self.bb_table.item(idx, 0).text():
                if hasattr(self.bb_table.item(idx, 0), "id"):
                    ...
                    order["p_id"] = database.db.get_id_by_code(
                        "product", self.bb_table.item(idx, 0).text()
                    )
...
class AppMainWindow(QtWidgets.QMainWindow):
    ...
    def setup_login(self):
        ...
        self.coin_exchange_from.currentTextChanged.connect(
            self.coin_exchange_from_change
        )

        self.listWidget.currentRowChanged.connect(
            lambda: self.listWidget_change(self.listWidget.currentRow())
        )
        ...
        self.p_class.currentTextChanged.connect(
            lambda: self.p_class_changed(self.p_class)
        )
        self.p_class_search.currentTextChanged.connect(
            lambda: self.p_class_changed(self.p_class_search)
        )
        ...
        self.p_table.clicked.connect(
            lambda mi: self.one_click_p(self.p_table.item(mi.row(), 0).id)
        )

        # search
        self.p_code_search.textChanged.connect(
            lambda text: self._typing_timer_p.start(1000)
        )
        self.p_name_search.textChanged.connect(
            lambda text: self._typing_timer_p.start(1000)
        )
        self.p_class_search.currentTextChanged.connect(
            lambda text: self._typing_timer_p.start(1000)
        )
        self.p_type_search.currentTextChanged.connect(
            lambda text: self._typing_timer_p.start(1000)
        )
        ...
        # pages
        self.p_post.clicked.connect(
            lambda: self.p_page_num.setValue(self.p_page_num.value() + 1)
        )
        self.p_previous.clicked.connect(
            lambda: self.p_page_num.setValue(self.p_page_num.value() - 1)
        )
        self.p_last.clicked.connect(
            lambda: self.p_page_num.setValue(
                math.ceil(
                    int(database.db.count_row("product", 1)) / self.p_page_size.value()
                )
            )
        )
    ...
    def update_users(self):
        ...
        for row_idx, row in enumerate(rows):
            ...
            if row_idx != 0:
                btn_delete = QtWidgets.QPushButton(QtGui.QIcon.fromTheme("delete"), "")
                btn_delete.clicked.connect(
                    lambda: self.users_table.removeRow(self.users_table.currentRow())
                )
        ...
        for row in range(self.users_table.rowCount()):
            ...
            if self.users_table.item(row, 0) and self.users_table.item(row, 0).text():
                ...
                if (
                    ...
                    QtWidgets.QMessageBox.warning(
                        None, "خطأ", "لا يجب أن تدخل كلمة مرور للمستخدم"
                    )
                    return
                if (
                    ...
                    QtWidgets.QMessageBox.warning(
                        None, "خطأ", "لا يجب أن تدخل صلاحيات للمستخدم"
                    )
                    return
                users.append(user)
        database.db.insert_users(users)
        toaster_Notify.QToaster.show_message(
            parent=self, message="تم حفظ التعديلات بنجاح"
        )
    ...
    def change_page_size(self, table):
        if table == "product":
            ...
            self.page_num_c.setRange(
                1,
                math.ceil(
                    int(database.db.count_customer(1)) / self.c_page_size.value()
                ),
            )
            self.update_customer_table()
        elif table == "supplier":
            self.s_page_num.setRange(
                1,
                math.ceil(
                    int(database.db.count_row("supplier", 1)) / self.s_page_size.value()
                ),
            )
            self.update_supplier_table()
        elif table == "partners":
            self.pa_page_num.setRange(
                1,
                math.ceil(
                    int(database.db.count_row("partners", 1))
                    / self.pa_page_size.value()
                ),
            )
            self.update_partners_table()
        elif table == "bill_sell":
            self.bs_page_num.setRange(
                1,
                math.ceil(
                    int(database.db.count_row("bill_sell", 1))
                    / self.bs_page_size.value()
                ),
            )
            self.update_bill_sell_table()
        elif table == "bill_buy":
            self.bb_page_num.setRange(
                1,
                math.ceil(
                    int(database.db.count_row("bill_buy", 1))
                    / self.bb_page_size.value()
                ),
            )
            self.update_bill_buy_table()
        elif table == "fund_movement":
            self.fm_page_num.setRange(
                1,
                math.ceil(
                    int(database.db.count_row("fund_movement", 1))
    ...
    def check_date_from(self, x):
        if x == "bell_sell":
            ...
            if self.ch_billsell_date_from.isChecked():
                ...
                self.billsell_date_from.dateChanged.connect(
                    lambda: self._typing_timer_bs.start(1000)
                )
            ...
            if self.ch_billbuy_date_from.isChecked():
                ...
                self.billbuy_date_from.dateChanged.connect(
                    lambda: self._typing_timer_bb.start(1000)
                )
            ...
            if self.ch_fm_date_from.isChecked():
                ...
                self.fm_date_from.dateChanged.connect(
                    lambda: self._typing_timer_fm.start(1000)
                )
    ...
    def check_date_to(self, x):
        if x == "bell_sell":
            ...
            if self.ch_billsell_date_to.isChecked():
                ...
                self.billsell_date_to.dateChanged.connect(
                    lambda: self._typing_timer_bs.start(1000)
                )
            else:
                self.billsell_date_to.setEnabled(False)
            ...
            if self.ch_billbuy_date_to.isChecked():
                ...
                self.billbuy_date_to.dateChanged.connect(
                    lambda: self._typing_timer_bb.start(1000)
                )
            else:
                self.billbuy_date_to.setEnabled(False)
            ...
            if self.ch_fm_date_to.isChecked():
                ...
                self.fm_date_to.dateChanged.connect(
                    lambda: self._typing_timer_fm.start(1000)
                )
            else:
                self.fm_date_to.setEnabled(False)
    ...
    def print_table_product(self):
        fil = self.search_product_save()
        products = database.db.query_all_product(
            fil, 0, database.db.count_row("product", 1)
        )
        with open("./html/product_template.html", "r") as f:
            template = Template(f.read())
            fp = tempfile.NamedTemporaryFile(
                mode="w", delete=False, dir="./html/tmp/", suffix=".html"
            )
            for idx, product in enumerate(products):
                product["idx"] = idx + 1

            html = template.render(
                products=products, date=time.strftime("%A, %d %B %Y %I:%M %p")
            )
            html = html.replace("style.css", "../style.css").replace(
                "ph1.png", "../ph1.png"
            )
            fp.write(html)
            fp.close()
    ...
    def setup_controls_customer(self):
        ...
        self.c_table.clicked.connect(
            lambda mi: self.one_click_c(self.c_table.item(mi.row(), 0).id)
        )
        self.c_page_num.setRange(
            1,
            math.ceil(
                int(database.db.count_row("customer", 1)) / self.c_page_size.value()
            ),
        )

        # search
        self.c_code_search.textChanged.connect(
            lambda text: self._typing_timer_c.start(1000)
        )
        self.c_name_search.textChanged.connect(
            lambda text: self._typing_timer_c.start(1000)
        )
        ...
        # pages
        self.c_post.clicked.connect(
            lambda: self.c_page_num.setValue(self.c_page_num.value() + 1)
        )
        self.c_previous.clicked.connect(
            lambda: self.c_page_num.setValue(self.c_page_num.value() - 1)
        )
        self.c_last.clicked.connect(
            lambda: self.c_page_num.setValue(
                math.ceil(
                    int(database.db.count_row("customer", 1)) / self.c_page_size.value()
                )
            )
    ...
    def delete_customer(self):
        ...
        button_reply = msg.question(
            self,
            "تأكيد",
            f"هل أنت متأكد من حذف {customer['name']} ؟ ",
            msg.Yes | msg.No,
            msg.No,
        )
        if button_reply == msg.Yes:
    ...
    def print_table_customer(self):
        fil = self.search_customer_save()
        customer = database.db.query_all_cs(
            "customer", fil, 0, database.db.count_row("customer", 1)
        )
        with open("./html/customer_template.html", "r") as f:
            ...
            html = template.render(
                customer=customer, date=time.strftime("%A, %d %B %Y %I:%M %p")
            )
            html = html.replace("style.css", "../style.css").replace(
                "ph1.png", "../ph1.png"
            )
    ...
    def setup_controls_supplier(self):
        ...
        self.s_table.clicked.connect(
            lambda mi: self.one_click_s(self.s_table.item(mi.row(), 0).id)
        )
        self.s_page_num.setRange(
            1,
            math.ceil(
        ...
        # search
        self.s_code_search.textChanged.connect(
            lambda text: self._typing_timer_s.start(1000)
        )
        self.s_name_search.textChanged.connect(
            lambda text: self._typing_timer_s.start(1000)
        )
        ...
        # pages
        self.s_post.clicked.connect(
            lambda: self.s_page_num.setValue(self.s_page_num.value() + 1)
        )
        self.s_previous.clicked.connect(
            lambda: self.s_page_num.setValue(self.s_page_num.value() - 1)
        )
        self.s_last.clicked.connect(
            lambda: self.s_page_num.setValue(
                math.ceil(
                    int(database.db.count_row("supplier", 1)) / self.s_page_size.value()
                )
            )
    ...
    def delete_supplier(self):
        ...
        button_reply = msg.question(
            self,
            "تأكيد",
            f"هل أنت متأكد من حذف {supplier['name']} ؟ ",
            msg.Yes | msg.No,
            msg.No,
        )
        if button_reply == msg.Yes:
    ...
    def print_table_supplier(self):
        fil = self.search_supplier_save()
        suppliers = database.db.query_all_cs(
            "supplier", fil, 0, database.db.count_row("supplier", 1)
        )
        with open("./html/supplier_template.html", "r") as f:
            ...
            html = template.render(
                suppliers=suppliers, date=time.strftime("%A, %d %B %Y %I:%M %p")
            )
            html = html.replace("style.css", "../style.css").replace(
                "ph1.png", "../ph1.png"
            )
    ...
    def setup_controls_partners(self):
        ...
        self.pa_table.clicked.connect(
            lambda mi: self.one_click_pa(self.pa_table.item(mi.row(), 0).id)
        )
        self.pa_page_num.setRange(
            1,
            math.ceil(
                int(database.db.count_row("partners", 1)) / self.pa_page_size.value()
            ),
        )

        # search
        self.pa_code_search.textChanged.connect(
            lambda text: self._typing_timer_pa.start(1000)
        )
        self.pa_name_search.textChanged.connect(
            lambda text: self._typing_timer_pa.start(1000)
        )

        self.pa_page_num.valueChanged.connect(self.update_partners_table)
        self.pa_page_size.valueChanged.connect(
            lambda: self.change_page_size("partners")
        )
        ...
        # pages
        self.pa_post.clicked.connect(
            lambda: self.pa_page_num.setValue(self.pa_page_num.value() + 1)
        )
        self.pa_previous.clicked.connect(
            lambda: self.pa_page_num.setValue(self.pa_page_num.value() - 1)
        )
        self.pa_last.clicked.connect(
            lambda: self.pa_page_num.setValue(
                math.ceil(
                    int(database.db.count_row("partners", 1))
                    / self.pa_page_size.value()
                )
    ...
    def delete_partners(self):
        ...
        button_reply = msg.question(
            self,
            "تأكيد",
            f"هل أنت متأكد من حذف {partners['name']} ؟ ",
            msg.Yes | msg.No,
            msg.No,
        )
        if button_reply == msg.Yes:
    ...
    def print_table_partners(self):
        fil = self.search_partners_save()
        partners = database.db.query_all_cs(
            "partners", fil, 0, database.db.count_row("partners", 1)
        )
        with open("./html/partners_template.html", "r") as f:
            ...
            html = template.render(
                partners=partners, date=time.strftime("%A, %d %B %Y %I:%M %p")
            )
            html = html.replace("style.css", "../style.css").replace(
                "ph1.png", "../ph1.png"
            )
    ...
    def setup_controls_bill_sell(self):
        ...
        self.bs_table.clicked.connect(
            lambda mi: self.one_click_bs(self.bs_table.item(mi.row(), 0).id)
        )
        self.bs_page_num.setRange(
            1,
            math.ceil(
                int(database.db.count_row("bill_sell", 1)) / self.bs_page_size.value()
            ),
        )

        self.billsell_code.textChanged.connect(
            lambda text: self._typing_timer_bs.start(1000)
        )
        self.billsell_cname.currentTextChanged.connect(
            lambda text: self._typing_timer_bs.start(1000)
        )

        self.ch_billsell_date_from.toggled.connect(
            lambda: self.check_date_from("bell_sell")
        )
        self.ch_billsell_date_to.toggled.connect(
            lambda: self.check_date_to("bell_sell")
        )

        self.bs_page_num.valueChanged.connect(self.update_bill_sell_table)
        self.bs_page_size.valueChanged.connect(
            lambda: self.change_page_size("bell_sell")
        )
        ...
        # pages
        self.bs_post.clicked.connect(
            lambda: self.bs_page_num.setValue(self.bs_page_num.value() + 1)
        )
        self.bs_previous.clicked.connect(
            lambda: self.bs_page_num.setValue(self.bs_page_num.value() - 1)
        )
        self.bs_last.clicked.connect(
            lambda: self.bs_page_num.setValue(
                math.ceil(
                    int(database.db.count_row("bell_sell", 1))
                    / self.page_size_bell_sell
                )
            )
        )
        ...
        self.btn_edit_billsell.clicked.connect(
            lambda: self.open_bill_sell(self.bill_sell_id)
        )
    ...
    def open_bill_sell(self, id):
        ...
        if DOLLAR == 0:
            QtWidgets.QMessageBox.warning(
                None, "خطأ", "أدخل سعر تصريف الدولار بالليرة التركية من الصفحة الرئيسية"
            )
            return
        sb = BillSell(id)
    ...
    def search_bill_sell_save(self):
        ...
        if self.billsell_cname.currentText() != "":
            fil["c_id"] = [
                ...
                if v == self.billsell_cname.currentText()
            ][0]
        if self.ch_billsell_date_from.isChecked():
    ...
    def update_bill_sell_table(self):
        ...
        rows = database.db.query_all_bill(
            "bill_sell",
            fil,
            self.bs_page_size.value() * (self.bs_page_num.value() - 1),
            self.bs_page_size.value(),
        )
        ...
        for row_idx, row in enumerate(rows):
            ...
            total = str(
                round(
                    float(row["paid_d"])
                    + (float(row["paid_t"]) / float(row["dollar_tr"])),
                    2,
                )
            )
    ...
    def print_table_bill_sell(self):
        fil = self.search_bill_sell_save()
        bills = database.db.query_all_bill(
            "bill_sell", fil, 0, database.db.count_row("bill_sell", 1)
        )
        with open("./html/bill_sell_template.html", "r") as f:
            ...
            html = template.render(
                bills=bills, date=time.strftime("%A, %d %B %Y %I:%M %p")
            )
            html = html.replace("style.css", "../style.css").replace(
                "ph1.png", "../ph1.png"
            )
    ...
    def setup_controls_bill_buy(self):
        ...
        self.bb_table.clicked.connect(
            lambda mi: self.one_click_bb(self.bb_table.item(mi.row(), 0).id)
        )
        self.bb_page_num.setRange(
            1,
            math.ceil(
                int(database.db.count_row("bill_buy", 1)) / self.bb_page_size.value()
            ),
        )

        self.billbuy_code.textChanged.connect(
            lambda text: self._typing_timer_bb.start(1000)
        )
        self.billbuy_sname.currentTextChanged.connect(
            lambda text: self._typing_timer_bb.start(1000)
        )

        self.ch_billbuy_date_from.toggled.connect(
            lambda: self.check_date_from("bell_buy")
        )
        ...
        self.bb_page_size.valueChanged.connect(
            lambda: self.change_page_size("bell_buy")
        )
        ...
        # pages
        self.bb_post.clicked.connect(
            lambda: self.bb_page_num.setValue(self.bb_page_num.value() + 1)
        )
        self.bb_previous.clicked.connect(
            lambda: self.bb_page_num.setValue(self.bb_page_num.value() - 1)
        )
        self.bb_last.clicked.connect(
            lambda: self.bb_page_num.setValue(
                math.ceil(
                    int(database.db.count_row("bell_buy", 1)) / self.page_size_bell_buy
                )
            )
        )
        ...
        self.btn_edit_billbuy.clicked.connect(
            lambda: self.open_bill_buy(self.bill_buy_id)
        )
    ...
    def search_bill_buy_save(self):
        ...
        if self.billbuy_sname.currentText() != "":
            fil["s_id"] = [
                ...
                if v == self.billbuy_sname.currentText()
            ][0]
        if self.ch_billbuy_date_from.isChecked():
    ...
    def update_bill_buy_table(self):
        ...
        rows = database.db.query_all_bill(
            "bill_buy",
            fil,
            self.bb_page_size.value() * (self.bb_page_num.value() - 1),
            self.bb_page_size.value(),
        )
    ...
    def print_table_bill_buy(self):
        fil = self.search_bill_sell_save()
        bills = database.db.query_all_bill(
            "bill_buy", fil, 0, database.db.count_row("bill_buy", 1)
        )
    ...
    def setup_controls_fund_movement(self):
        ...
        self.fm_table.clicked.connect(
            lambda mi: self.one_click_fm(self.fm_table.item(mi.row(), 0).id)
        )
        self.fm_page_num.setRange(
            1,
            math.ceil(
        ...
        self.s_fm_owner.currentTextChanged.connect(
            lambda text: self._typing_timer_fm.start(1000)
        )
        self.ch_fm_date_from.toggled.connect(
            lambda: self.check_date_from("fund_movement")
        )
        ...
        self.fm_page_size.valueChanged.connect(
            lambda: self.change_page_size("fund_movement")
        )
        ...
        # pages
        self.fm_post.clicked.connect(
            lambda: self.fm_page_num.setValue(self.fm_page_num.value() + 1)
        )
        self.fm_previous.clicked.connect(
            lambda: self.fm_page_num.setValue(self.fm_page_num.value() - 1)
        )
        self.fm_last.clicked.connect(
            lambda: self.fm_page_num.setValue(
                math.ceil(
                    int(database.db.count_row("fund_movement", 1))
                    / self.fm_page_size.value()
                )
        ...
        if self.fm_type.currentIndex() == 1:
            if self.fm_owner.currentText() != "":
                cid = [
                    k
                    for k, v in self.customers.items()
                    if v == self.fm_owner.currentText()
                ][0]
                self.fm_discount.setText(
                    database.db.query_row("customer", cid)["balance"]
                )
        elif self.fm_type.currentIndex() == 2:
            if self.fm_owner.currentText() != "":
                cid = [
                ...
                self.fm_discount.setText(
                    database.db.query_row("supplier", cid)["balance"]
                )
    ...
    def save_fm_info(self) -> dict:
        ...
        if self.fm_type.currentIndex() == 1:
            fm["owner"] = [
                k for k, v in self.customers.items() if v == self.fm_owner.currentText()
            ][0]
        elif self.fm_type.currentIndex() == 2:
            fm["owner"] = [
                k for k, v in self.suppliers.items() if v == self.fm_owner.currentText()
            ][0]
        elif self.fm_type.currentIndex() in [3, 4]:
            fm["owner"] = [
                k for k, v in self.partners.items() if v == self.fm_owner.currentText()
            ][0]
    ...
    def update_fm(self):
        ...
        if fm["type"]:
            ...
            self.setup_box()
            toaster_Notify.QToaster.show_message(
                parent=self, message=f"تعديل حركة\nتم تعديل الحركة {fm['type']} بنجاح"
            )
        else:
            QtWidgets.QMessageBox.warning(None, "خطأ", "يجب أن تدخل نوع الحركة")
    ...
    def delete_fm(self):
        msg = QtWidgets.QMessageBox()
        button_reply = msg.question(
            self, "تأكيد", f"هل أنت متأكد من حذف هذه الحركة؟ ", msg.Yes | msg.No, msg.No
        )
        if button_reply == msg.Yes:
        ...
        if self.s_fm_type.currentIndex() != 0:
            ...
            if self.s_fm_type.currentIndex() == 1 and self.s_fm_owner.currentText():
                fil["owner"] = [
                    k
                    for k, v in self.customers.items()
                    if v == self.s_fm_owner.currentText()
                ][0]
    ...
    def print_table_fm(self):
        fil = self.search_fm_save()
        fms = database.db.query_all_fm(
            fil, 0, database.db.count_row("fund_movement", 1)
        )
            ...
            self.notif_table.setItem(
                ...
                QtWidgets.QTableWidgetItem(
                    str(
                        row_idx
                        + 1
                        + (self.n_page_size.value() * (self.n_page_num.value() - 1))
                    )
                ),
            )
