...
class Database:
    ...
    def change_user_pass(self, username, password):
        self.connection.execute(
            "UPDATE users SET pass = ? WHERE users.name = ?", (password, username)
        )
    ...
    def is_user(self, username):
        return self.connection.execute(
            "select pass, permission from users where name = ?", (username,)
        ).fetchone()
    ...
    def count_row(self, table, r):
        if r == 1:
            ...
            return self.connection.execute(
                f"select count(*) as count from {table} where code = ?", (r,)
            ).fetchone()["count"]

    def get_next_id(self, table):
        if (
            self.connection.execute(f"select max(id)+1 as seq from {table}").fetchone()[
                "seq"
            ]
            == ""
        ):
            return 1
        return self.connection.execute(
            f"select max(id)+1 as seq from {table}"
        ).fetchone()["seq"]
    ...
    def query_csp(self, table):
        return {
            e["id"]: e["name"]
            for e in self.connection.execute(f"select id, name from {table}").fetchall()
        }
    ...
    def get_code_by_id(self, table, id):
        return self.connection.execute(
            f"select code from {table} where id = ?", (id,)
        ).fetchone()["code"]

    def count_table(self, table, id):
        return self.connection.execute(
            f"SELECT count(*) as count FROM {table} where id = ?", (id,)
        ).fetchone()["count"]
    ...
    def get_earnings(self, now, month):
        ...
        cost = self.connection.execute(
            ...
            "b_id in (SELECT id FROM bill_sell WHERE date <= ? and date > ?)",
            (now, month),
        ).fetchone()["cost"]
        cost = float(cost or 0)

        naf = self.connection.execute(
            "SELECT value + (value_t / do_tr) as naf FROM fund_movement WHERE type = 'نفقات' and date <= ? and date > ?",
            (now, month),
        ).fetchone()
    ...
    def get_sales(self, now, month):
        return self.connection.execute(
            "SELECT sum(paid_d + paid_t / dollar_tr) as sales FROM bill_sell WHERE date <= ? and date > ?",
            (now, month),
        ).fetchone()["sales"]

    def get_purchases(self, now, month):
        return self.connection.execute(
            "SELECT sum(total - discount) as purchases FROM bill_buy WHERE date <= ? and date > ?",
            (now, month),
        ).fetchone()["purchases"]

    def get_capital(self, with_box, do_tr) -> float:
        pro_capital = self.connection.execute(
            "SELECT sum(quantity * buy_price) as pro_capital FROM product"
        ).fetchone()["pro_capital"]
        if pro_capital == "":
            pro_capital = "0"
        if do_tr == 0:
            box = self.connection.execute("SELECT dollar as box FROM box").fetchone()[
                "box"
            ]
        else:
            box = self.connection.execute(
                "SELECT dollar + (turky / ?) as box FROM box", (do_tr,)
            ).fetchone()["box"]
        if with_box:
    ...
    def insert_table(self, table, dic, fk):
        ...
        del_ids = self.connection.execute(
            f"SELECT id FROM {table} WHERE b_id = ? and id not in ({placeholders})",
            tuple([fk, *new_ids]),
        ).fetchall()
        for d in dic:
    ...
    def insert_users(self, lis):
        ...
        placeholders = ", ".join("?" * len(new_ids))
        del_ids = self.connection.execute(
            f"SELECT id FROM users WHERE id not in ({placeholders})", tuple(new_ids)
        ).fetchall()
        for d in lis:
    ...
    def get_phone_by_name(self, table, name):
        return self.connection.execute(
            f"select phone from {table} where name = ?", (name,)
        ).fetchone()["phone"]
    ...
    def get_balance(self, type_fm, owner):
        if type_fm == "دفعة من زبون":
            return self.connection.execute(
                "SELECT balance FROM customer WHERE id = ?", (owner,)
            ).fetchone()["balance"]
        else:
            return self.connection.execute(
                "SELECT balance FROM supplier WHERE id = ?", (owner,)
            ).fetchone()["balance"]
    ...
    def get_order_bill(self, table, b_id):
        return self.connection.execute(
            f"select * FROM {table} WHERE b_id = ?", (b_id,)
        ).fetchall()

    def get_noti_pro1(self):
        return self.connection.execute(
            f"SELECT code, name, quantity FROM product WHERE quantity <= less_quantity"
        ).fetchall()

    def get_noti_pro2(self):
        return self.connection.execute(
            f"SELECT code, name, quantity FROM product WHERE quantity = 0"
        ).fetchall()

    def get_noti_cus(self, table):
        return self.connection.execute(
            f"SELECT code, name, range_balance FROM {table} WHERE balance >= range_balance"
        ).fetchall()
    ...
    def exchange_dollar_turky(self, to, do, tu):
        if to == "do_tu":
            self.connection.execute(
                "UPDATE box SET dollar = dollar - ?, turky = turky + ?", (do, tu)
            )
        else:
            self.connection.execute(
                "UPDATE box SET dollar = dollar + ?, turky = turky - ?", (do, tu)
